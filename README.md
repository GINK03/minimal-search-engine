...
出力が、単語ごとにテキストファイルが作成されて、 `URLのハッシュ` , `weight(tfidf)` , `refnum(被参照数)` のファイルが具体的な転置インデックスのファイルになる
```
...
本来は、謎のヒューリスティックに依存した掛け算やlogを取るアプローチより、正解の並び順を与えて、機械学習で正しい並び順になるように学習するのが筋が良いように思います。

SEOという文化が登場してもう何十年にもなると思うのですが、あれのせいか、高いweightを非参照リンクなどに与えるとひどいことになります。具体的には、ほぼアフィリエイトブログやアドネットワークのようなものが大量に上に来てしまい、使い物になりません。これがGoogle以外のエンジンが全滅した一因に思いました。  
